...
import { updateKeyword, triggerEngineScrape } from "../../actions";
...
export function EditKeywordForm({ keyword }: { keyword: any }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    // Step 1: Update the keyword
    const result = await updateKeyword(keyword.id, formData);
    if (result?.error) {
    ...
    <div className="mx-auto max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="mt-4 text-2xl font-bold tracking-tight text-foreground">
          Edit Keyword Tracker
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Update your keyword configuration.
        </p>
      </div>
      ...
      {loadingStep === "fetching" && (
        <div className="mb-6 animate-fade-in rounded-xl border border-accent/20 bg-accent/5 px-4 py-3 text-sm text-accent-light flex items-center gap-3">
          ...
          Fetching new posts with updated settings...
        </div>
      )}
        ...
        <form action={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              type="text"
              defaultValue={keyword.target_subreddits.join(", ")}
              required
              disabled={loadingStep !== "idle"}
          ...
          <div className="flex flex-col gap-1.5">
            <Input
              ...
              type="url"
              defaultValue={keyword.discord_webhook_url || ""}
              disabled={loadingStep !== "idle"}
            />
            <p className="text-xs text-muted">
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={loadingStep !== "idle"}>
              {loadingStep === "saving" ? "Saving…" : loadingStep === "fetching" ? "Scanning Reddit…" : "Save Changes"}
            </Button>
